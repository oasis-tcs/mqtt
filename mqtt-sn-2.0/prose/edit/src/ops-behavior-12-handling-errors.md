...
Definitions of Malformed Packet and Protocol Errors are contained in [sec](#terminology), some but not all of these error cases are noted throughout the specification. The rigor with which a Client or Server checks an MQTT-SN Control Packet it has received will be a compromise between:
...
Refer to [sec](#handling-of-disallowed-unicode-code-points) for information about handling Disallowed Unicode code points.
