...
APPENDIX_HEAD_REMAP = {
    '# Appendix A. Acknowledgments': {
        'prepend': [r'\newpage', ''],
        'attrs': '{.unnumbered #appendix-a.-acknowledgments}'
    },
    '## A.1 Special Thanks': {'attrs': '{.unnumbered #a.1-special-thanks}'},
    '## A.2 Participants': {'attrs': '{.unnumbered #a.2-participants}'},
    '# Appendix B. Mandatory normative statements (informative)': {
        'prepend': [r'\newpage', ''],
        'attrs': '{.unnumbered #appendix-b.-mandatory-normative-statements-informative}'
    },
    '# Appendix C. Implementation Guidance (Informative)': {
        'prepend': [r'\newpage', ''],
        'attrs': '{.unnumbered #appendix-c.-implementation-guidance-informative}',
    },
    '## C.1 Example MQTT-SN Architectures': {
        'attrs': '{.unnumbered #c.1-example-mqtt-sn-architectures}'
    },
    '### C.1.1 Transparent Gateway': {'attrs': '{.unnumbered #c.1.1-transparent-gateway}'},
    '### C.1.2 Aggregating Gateway': {'attrs': '{.unnumbered #c.1.2-aggregating-gateway}'},
    '### C.1.3 Forwarder': {'attrs': '{.unnumbered #c.1.3-forwarder}'},
    '### C.1.4 MQTT-SN Broker': {'attrs': '{.unnumbered #c.1.4-mqtt-sn-broker}'},
    '## C.2 Server Congestion': {'attrs': '{.unnumbered #c.2-server-congestion}'},
    '## C.3 Example Timer and Counter Values': {
        'attrs': '{.unnumbered #c.3-example-timer-and-counter-values}'
    },
    '## C.4 Exponential Backoff': {'attrs': '{.unnumbered #c.4-exponential-backoff}'},
    '## C.5 Client State Diagrams': {'attrs': '{.unnumbered #c.5-client-state-diagrams}'},
    '## C.6 PUBLISH with QoS -1': {'attrs': '{.unnumbered #c.6-publish-with-qos--1}'},
    '### C.6.1 PUBLISH Header': {'attrs': '{.unnumbered #c.6.1-publish-header}'},
    '### C.6.2 PUBLISH Flags': {'attrs': '{.unnumbered #c.6.2-publish-flags}'},

    '#### C.6.2.1 Topic Type': {'attrs': '{.unnumbered #c.6.2.1-topic-type}'},
    '#### C.6.2.2 QoS': {'attrs': '{.unnumbered #c.6.2.2-qos}'},
    '#### C.6.2.3 DUP': {'attrs': '{.unnumbered #c.6.2.3-dup}'},
    '#### C.6.2.4 Retain': {'attrs': '{.unnumbered #c.6.2.4-retain}'},

    '### C.6.3 Topic Alias': {'attrs': '{.unnumbered #c.6.3-topic-alias}'},
    '### C.6.4 Topic Short Name': {'attrs': '{.unnumbered #c.6.4-topic-short-name}'},
    '### C.6.5 Topic Name Length': {'attrs': '{.unnumbered #c.6.5-topic-name-length}'},
    '### C.6.6 Topic Name': {'attrs': '{.unnumbered #c.6.6-topic-name}'},
    '### C.6.7 Payload': {'attrs': '{.unnumbered #c.6.7-payload}'},
    '### C.6.8 PUBLISH with QoS -1 Actions': {
        'attrs': '{.unnumbered #c.6.8-publish-with-qos--1-actions}'
    },
    '## C.7 Gateway Advertisement and Discovery': {
        'attrs': '{.unnumbered #c.7-gateway-advertisement-and-discovery}'
    },
    '# Appendix D. Revision History (informative)': {
        'prepend': [r'\newpage', ''],
        'attrs': '{.unnumbered #revision-history-informative}'
    },
}
...
def insert_any_section_reference(record: str, heading_label_lut: dict[str, str], heading_text_lut: dict[str, str]) -> str:
    """Insert section reference into section ref placeholder or return record unchanged."""
    heading_not_found_marker = 'HEADING-NOT-FOUND'
    if label_in(record):
        for ref in SEC_REF_DETECT.finditer(record):
            if ref:
                ...
                label = found['label']
                if label not in heading_label_lut:
                ...
                text = heading_label_lut[label]

                heading_text = heading_text_lut.get(text, heading_not_found_marker)
                if heading_text == heading_not_found_marker:
                    print(f'WARNING: in insert-any-section-reference-heading-text ({record=})')
                    print(f'WARNING-CONTEXT: {record=} - {trigger_text=}')
                    print(f'WARNING-CONTEXT: {record=} - {label=}')
                    for hkey in heading_text_lut:
                        if hkey.startswith(label[:len(label) // 2]):
                            print(f'DEBUG: - similar {hkey=} exists')
                    print(f'DEBUG: You may want to execute grep -n {label} src/*.md')
...
def main(args: list[str]) -> int:
        ...
        for tag in sec_cnt:
            if line.startswith(tag) and clean_headings and not in_fenced_block:
                ...
                if terse_line in APPENDIX_HEAD_REMAP:
                    ...
                    if 'prepend' in transform:  # type: ignore
                        lines[slot - 1] = lines[slot - 1] + NL + NL.join(transform['prepend'])  # type: ignore
                    if 'replace' in transform:  # type: ignore
                        this, that = transform['replace']  # type: ignore
    ...
    for slot, line in enumerate(lines):
        completed = insert_any_section_reference(line, display_from, heading_text_of)
        if line != completed:
            lines[slot] = completed
